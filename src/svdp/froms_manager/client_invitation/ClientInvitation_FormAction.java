 ...
 */
@WebServlet("/ClientInvitation_FormAction")
public class ClientInvitation_FormAction extends HttpServlet 
{
     ...
     */
    public ClientInvitation_FormAction() 
    {
        super();
    }
	...
	protected String reply(String messageID, String senderID, String receiverID, Map<String, String[]> parameterMap) throws IOException
	{
		...
		try
		{
			long   hhhID 	= Long.parseLong( receiverID.substring( 2 ), 16 );
			String query 	= "SELECT Email FROM Hauseholdheads WHERE HauseholdheadID=" + hhhID ;
			String hhhMail 	= mysql.simpleQuery( query );
			
			String state 	= accepted ? "ACCEPTED" : "REJECTED";
			String command 	= "UPDATE ClientsInvitedToEvents SET Status=\"" + state + "\" WHERE HauseholdheadEmail=\"" + hhhMail + "\" AND EventName=\"" + event_name + "\";"; 
			...
			if ( accepted )
			{
				svdp.general.Util.EventInfo ee = Util.getEventInfo( mysql, event_name );
